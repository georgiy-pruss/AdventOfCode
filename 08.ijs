NB. count string chars http://adventofcode.com/day/8

split1=: <;._2@,~
dcr=: ] }.~ [: - CR = {:  NB. drop last cr; dtb = drop trailing blanks

main =: 3 : 0
  ll=.ls=.ln=.0 NB. length of line, string value, new encoded string
  for_l. y do.
    if. 0=# l=. dtb dcr >l do. continue. end.
    ll=.ll+# o=.l NB. save original line in o for part 2
    assert ('"'={.l) *. '"'={:l
    l=.(}.}:l) rplc '\\';'_';'\"';'_'
    ls=.ls+(#l)-3*+/'\x' E. l
